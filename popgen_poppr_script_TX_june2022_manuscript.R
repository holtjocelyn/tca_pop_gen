...
obj <- read.structure("~/Desktop/tca_data/poppr3/tca_populations_tx_county_samples_structure_format_aug2022.stru", n.ind = 53, n.loc = 26657, onerowperind = FALSE, col.lab = 0, col.pop = 2, col.others = NULL, row.marknames = 1, NA.char = "-9", pop = NULL, sep = "\t", ask = TRUE, quiet = TRUE)
...
objmxclust <- find.clusters(obj)

#choose the number of PCs to retain = 15

#choose the number of clusters = 3, lowest value. After run K=5 low value with biological meaning, both low points in the dataset
...
objgenind <- import2genind("~/Desktop/tca_data/poppr3/tca_populations_tx_county_samples_structure_format_aug2022.stru", n.ind = 53, n.loc = 26657, onerowperind = FALSE, col.lab = 0, col.pop = 2, col.others = NULL, row.marknames = 1, NA.char = "-9", pop = NULL, sep = "\t", ask = TRUE, quiet = TRUE)
...
#the lowest value of k is 4, and the second lowest is 3, which makes biological sense given that ARG COL and PER pops are very different
#these values are slightly different when calculated off the genind vs. the stru file

#get approximately 11 PCs; this should be equal to or less than the actual pops have. if go too high in number than the discriminant analysis eigenvalues will not be as clear and the data points will not be visible on the scatter plot
...
9 #choose the number of discriminant functions to retain = 9. Also run with 7, since this is another possibility for the number of clusters
...
myCol <- c("darkblue", "purple", "green", "orange", "brown", "black", "grey", "red", "turquoise", "light blue", "pink", "yellow")
...
#set your working directory
setwd("~/Desktop/tca_data/poppr3/tca_populations_tx_county_samples_structure_format_aug2022.stru")
